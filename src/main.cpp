...
uint256 hashGenesisBlock("0x3779d52f9fb60c1497e6d488995700a630c1f777c3da7a688da440f9377cbddb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bapcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bapcoin Signed Message:\n";
...
{
    // Bapcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Bapcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Bapcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Bapcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bapcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bapcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Bapcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3779d52f9fb60c1497e6d488995700a630c1f777c3da7a688da440f9377cbddb");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "First day of bapcoin85";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x6cd4d1a831e2c5faf7b2156f7485433fb1c0f7f4208523a6b0a6268b32cf8295"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Bapcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BapcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BapcoinMiner : ProcessBlock, block not accepted");
    }
